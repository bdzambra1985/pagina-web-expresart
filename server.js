...
const { getSessionByRawToken, restoreFromDB } = require('./middleware/auth');
...
const { pruneOldLogs, LOG_DIR: SECURITY_LOG_DIR } = require('./utils/securityLog');
...
const twoFactorRoutes  = require('./routes/twofactor').router;
...
async function initAdmin() {
    ...
    if (!existing) {
        ...
        await db.updateUser('admin', { passwordHash: await hashPassword(pw) });
...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            defaultSrc:     ["'self'"],
            scriptSrc:      ["'self'"],
            // unpkg.com eliminado: boxicons ahora se sirve desde /vendor.
            // Un CDN de terceros en styleSrc es una vía de entrada de código
            // ajeno al sitio si el CDN se ve comprometido.
...
/* ── CSRF: se aplica a toda mutación con sesión activa.
   Va después de los parsers de body para que los handlers exentos
   (webhook de Resend) tengan req.rawBody disponible. ── */
app.use(csrfProtection);

/* ── Static files ──
   Solo se sirve public/. Todo lo demás (código de servidor, backups, data/,
   .env, documentación interna) queda fuera del alcance del servidor estático
   por construcción, en vez de depender de una lista negra que hay que
   recordar actualizar cada vez que se añade un archivo a la raíz.
*/
...
app.use('/uploads/receipts', (req, res, next) => {
    const cookieTok = readSession(req);
...
app.use('/uploads', require('express').static(UPLOADS_DIR, {
    setHeaders(res, filePath) {
        res.setHeader('Cache-Control', `public, max-age=${ONE_WEEK}`);
        // Los PDFs subidos (comprobantes) se fuerzan a descarga: abrir un PDF
        // no confiable inline lo ejecuta en nuestro propio origen, que es el
        // vector clásico de PDF malicioso. Las imágenes se dejan inline porque
        // se muestran en el portafolio y ya pasan por validación de
        // magic-bytes + reencodado con sharp.
...
/* ── Factura/comprobante accesible sin prefijo /api ──
   Solo se exponen en la raíz las rutas de factura, no todo orderRoutes:
   montar el router entero aquí duplicaba cada endpoint de pedidos fuera
   del prefijo /api, donde apiLimiter no aplica.
*/
app.use('/', (req, res, next) =>
    req.path.startsWith('/factura/') ? orderRoutes(req, res, next) : next());
...
(async () => {
    try {
        ...
        cleanupLegacyFiles();
        await initAdmin();
        // Recupera sesiones y bloqueos de login del despliegue anterior.
        await restoreFromDB();
        ...
        /* ── Rotación del log de seguridad a las 03:00 ── */
        cron.schedule('0 3 * * *', () => pruneOldLogs(), { timezone: 'America/Guayaquil' });
        pruneOldLogs();
        console.log(`  Log de seguridad: ${SECURITY_LOG_DIR}`);
